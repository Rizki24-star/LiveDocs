  ...
  try {
    ...
    if (room) {
      ...
      await liveblocks.triggerInboxNotification({
        ...
        activityData: {
          userType,
          title: `You have been granted ${userType} acccess to the document by ${updatedBy.name}`,
...
export const removeCollaborator = async ({
  ...
  try {
    ...
    console.log(`Error happened while deleting room access: ${error}`);
  }
};
...
export const deleteDocument = async (roomId: string) => {
  try {
    ...
    console.log(`Error while deleting room access: ${error}`);
  }
};
